...
Deno.serve(async (request) => {
  ...
  return createRecap(Number(week));
});
// ============================================================
// ESPN DATEN ABRUFEN
// ============================================================
...
async function createRecap(week) {
  try {
    ...
    const highestScore = scores[0] || null;
    const lowestScore = scores[scores.length - 1] || null;
    // ----------------------------------------------------------
    // ENGSTES / DEUTLICHSTES SPIEL
    // ----------------------------------------------------------
    const sortedByMargin = [...games]
      .filter(g => g.winner && g.loser)
      .sort((a, b) => a.margin - b.margin);
    const closestGame = sortedByMargin[0] || null;
    const biggestBlowout =
      sortedByMargin[sortedByMargin.length - 1] || null;
    // ----------------------------------------------------------
    // TOP-SPIELER
    // ----------------------------------------------------------
    const players = [];
    for (const teamRoster of roster) {
      if (!Array.isArray(teamRoster.players)) {
        continue;
      }
    ...
    const teamScores = scores.map(s => ({
      ...s,
      teamData: teams.find(t => t.name === s.team)
    }));
    // ----------------------------------------------------------
    // POWER RANKING
    //
    ...
    // Points For + Bilanz
    //
    // Kein offizielles ESPN-Ranking,
    // sondern eine einfache Recap-Auswertung.
    // ----------------------------------------------------------
    ...
    for (const transaction of transactions) {
      ...
      transactionTypes[type] =
        (transactionTypes[type] || 0) + 1;
    }
    // ----------------------------------------------------------
    // RECAP TEXT
    // ----------------------------------------------------------
    const lines = [];
    lines.push(`🏈 ESPN FANTASY FOOTBALL – WEEK ${week} RECAP`);
    lines.push("");
    lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    lines.push("");
    // ----------------------------------------------------------
    ...
    for (const game of games) {
      ...
      if (homeScore > awayScore) {
        result =
          `🏆 ${home.team} gewinnt mit ${homeScore.toFixed(2)} : ${awayScore.toFixed(2)}`;
      } else if (awayScore > homeScore) {
        result =
          `🏆 ${away.team} gewinnt mit ${awayScore.toFixed(2)} : ${homeScore.toFixed(2)}`;
      } else {
        result =
          `🤝 Unentschieden ${homeScore.toFixed(2)} : ${awayScore.toFixed(2)}`;
      }
      lines.push(
        `${home.team} ${homeScore.toFixed(2)} – ${awayScore.toFixed(2)} ${away.team}`
      );
      lines.push(result);
      lines.push("");
    }
    ...
    if (closestGame) {
      ...
      lines.push("");
      lines.push(
        `${closestGame.home.team} ${Number(
          closestGame.home.score
        ).toFixed(2)} – ${Number(
          closestGame.away.score
        ).toFixed(2)} ${closestGame.away.team}`
      );
      lines.push(
        `Nur ${closestGame.margin.toFixed(2)} Punkte Unterschied.`
      );
      lines.push("");
    }
    // ----------------------------------------------------------
    ...
    if (biggestBlowout && biggestBlowout.margin > 0) {
      ...
      lines.push(
        ...
        `mit ${biggestBlowout.margin.toFixed(2)} Punkten Vorsprung.`
      );
      lines.push("");
    }
    // ----------------------------------------------------------
    ...
    // ----------------------------------------------------------
    if (highestScore) {
      lines.push("🚀 TOP SCORE DER WOCHE");
      lines.push("");
      lines.push(
        `${highestScore.team}: ${highestScore.score.toFixed(2)} Punkte`
      );
      lines.push("");
    }
    // ----------------------------------------------------------
    ...
    // ----------------------------------------------------------
    if (lowestScore) {
      lines.push("🥶 LOW SCORE DER WOCHE");
      lines.push("");
      lines.push(
        `${lowestScore.team}: ${lowestScore.score.toFixed(2)} Punkte`
      );
      lines.push("");
    }
    // ----------------------------------------------------------
    ...
    if (standings.length > 0) {
      ...
      standings.forEach((team, index) => {
        lines.push(
          ...
          `${team.ties ? `-${team.ties}` : ""} | ` +
          `${Number(team.pointsFor || 0).toFixed(2)} PF`
        );
      });
      lines.push("");
    }
    ...
    if (powerRanking.length > 0) {
      ...
      lines.push("");
      powerRanking.forEach((team, index) => {
        lines.push(
          `${index + 1}. ${team.team}`
        );
      });
      lines.push("");
      lines.push(
        "Hinweis: Das Power Ranking ist eine Recap-interne "
        + "Auswertung aus Bilanz und erzielten Punkten und "
        + "kein offizielles ESPN-Ranking."
      );
      lines.push("");
    }
    ...
    if (transactionCount === 0) {
      lines.push("Keine Transaktionen für diese Woche gefunden.");
    } else {
      lines.push(
        `${transactionCount} Transaktion(en) registriert.`
      );
    ...
    return json({
      error: "Recap konnte nicht erstellt werden",
